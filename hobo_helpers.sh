...
function hobo_help {
	...
	then 
		...
		echo "hb_edge [appname]       : new HOBO app named [appname] using Hobo edge from github (as plugin)"			
		echo "hb_edge_nogit [appname] : new HOBO app named [appname] using Hobo edge from github (as plugin) but with .git removed"			
		echo "hb_nogit                : Remove any .git dir from hobo app and hobo plugin within"
		echo "hb_noedge               : Remove Hobo plugin from hobo project"					
		echo "hb_edge_install         : install Hobo edge from github (as plugin) in existing Hobo project"			
		echo "hb_edge_install_nogit   : install Hobo edge from github (as plugin) in existing Hobo project but with .git removed"			
		echo "hb_init [appname]       : new HOBO app named [appname]. Run initial hobo migration NOW"		
		echo "hm *                    : gen hobo MIGRATION and run it NOW"
		echo "hm_now [name]           : gen named hobo MIGRATION and run it NOW"	
		...
		echo "hbv [name] [action]     : generate DRYML skeleton file for specific REST action of named model"		
		echo "hbv_all [name]          : generate DRYML skeleton file for all REST actions of named model"
	...
	then 
		echo "hb [appname]            : new HOBO app named [appname]"
		echo "hb_edge [appname]       : new HOBO app named [appname] using Hobo edge from github (as plugin)"			
		echo "hb_edge_nogit [appname] : new HOBO app named [appname] using Hobo edge from github (as plugin) but with the .git removed"			
		echo "hb_nogit                : Remove any .git dir from hobo app and hobo plugin within"
		echo "hb_noedge               : Remove Hobo plugin from hobo project"							
		echo "hb_edge_install         : install Hobo edge from github (as plugin) in existing Hobo project"			
		echo "hb_edge_install_nogit   : install Hobo edge from github (as plugin) in existing Hobo project but with .git removed"			
	...
	then 
		...
		echo "hm_now [name]           : generate named hobo migration and run migration NOW"	
		echo "hm_init [name]          : run initial hobo migration"			
	fi

	if [[ $context == 'model' ]]		 		
	then 
		echo "hmdl [name]              : generate hobo model resource"
		echo "hmdl_now [name]          : generate new hobo model resource and run migration NOW"
	fi

	if [[ $context == 'model_resource' ]]		 		
	then 
		echo "hmr [name]              : generate hobo model resource"
	...
	if [[ $context == 'view' ]]		 		
	then 
		echo "hbv [model name (plural)] [action]     : generate DRYML skeleton for specific REST action of named model"		
		echo "hbv_all [model name (plural)]          : generate DRYML skeleton for all REST actions of named model"
		echo ""
